...
- URL-synced filter state uses **nuqs** (`NuqsAdapter` is in `apps/web/app/layout.tsx`; see `app/mail/page.client.tsx` and `components/Wars/WarRoom/WarList.tsx` for the pattern):
  ...
  - In a page cached whole (ISR), wrap the consumer in `NuqsAdapter` from `nuqs/adapters/react`: the Next adapter's `useSearchParams()` drops its subtree out of the cached HTML, leaving only the loader (see `app/history/build/[build]/page.tsx`).
- ISR for a dynamic `[param]` route (Next 16.2): an unlisted param re-renders everything after `await params` on every request unless the segment has `generateStaticParams` (at least one entry — a placeholder the page 404s before querying when the DB is unreachable at build) and `experimental.partialFallbacks` is on (it is, in `next.config.mjs`). Reference: `app/history/build/[build]/page.tsx`; verify with `next start` that the second request carries `x-nextjs-cache: HIT`. An ISR route caches its `notFound()` output, so drop any catch-DB-error-into-`notFound()` first.
