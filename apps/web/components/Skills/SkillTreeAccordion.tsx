...
export const SkillTreeAccordion = memo(() => {
  ...
  return (
    <Container size="sm">
      <Accordion defaultValue={[]} variant="separated" multiple>
        {alphabeticallySortedGroupIds.map((groupId) => {
          ...
          return (
            <Accordion.Item value={groupId.toString()} key={groupId}>
              ...
              <Accordion.Panel>
                <Stack spacing="xs">
                  {alphabeticallySortedTypeIdsPerGroup[groupId]?.map(
                    (typeId) => {
                        ...
                        <Group key={typeId} position="apart">
                          ...
                          <Group>
                            <Text size="xs" color="dimmed">
                              ...
                              / {getSPNeededForLevel(type, 5).toLocaleString()}{" "}
                              SP
                            </Text>
                            <SkillBar
                              activeLevel={
                                characterSkill?.active_skill_level ?? 0
                              }
                            />
                          </Group>
                        </Group>
