...
export const CorporationTickerBadge = memo(({ corporationId, ...otherProps }: CorporationTickerBadgeProps) => {
  const { data } = useCorporation(corporationId ?? 0);
  return <UICorporationTickerBadge ticker={data?.data.ticker} {...otherProps} />;
});
