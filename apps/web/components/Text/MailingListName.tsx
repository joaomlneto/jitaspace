...
export const MailingListName = memo(({ characterId, mailingListId, ...otherProps }: MailingListNameProps) => {
  const { data } = useCharacterMailingLists(characterId ?? 0);
  const mailingList = data?.data.find((ml) => ml.mailing_list_id === mailingListId);
