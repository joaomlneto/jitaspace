...
async function fetchTypeAttributes(typeId: number) {
  const res = await getUniverseTypesTypeId(typeId);
  return (res.data.dogma_attributes ?? []).map((typeAttribute) => ({
    typeId,
...
async function fetchTypeEffects(typeId: number) {
  const res = await getUniverseTypesTypeId(typeId);
  return (res.data.dogma_effects ?? []).map((typeEffect) => ({
    typeId,
...
async function processTypeBatch(
  thisBatchTypeIds: number[],
    ...
    fetchRemoteEntries: async () =>
      Promise.all(
        thisBatchTypeIds.flatMap((typeId) =>
          limit(() => fetchTypeAttributes(typeId)),
        ),
...
export const scrapeEsiTypes = defineJob<ScrapeTypesEventPayload["data"]>({
  id: "scrape-esi-types",
  trigger: { type: "event" },
  name: "Scrape Types",
  concurrencyLimit: 1,
  ...
  handler: async (ctx) => {
    ...
    for (const [i, thisBatchTypeIds] of batches.entries()) {
      const result = await ctx.run(
        ...
        (): Promise<BatchStepResult<StatsKey>> =>
          processTypeBatch(thisBatchTypeIds, limit),
      );
      results.push(result);
    }
