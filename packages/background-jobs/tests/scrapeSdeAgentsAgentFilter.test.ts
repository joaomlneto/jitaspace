/**
 * The `agent`-block filter, one handler run per fixture.
 *
 * A sibling of `scrapeSdeAgents.test.ts`, which runs the handler exactly once
 * against a fixed archive and asserts over everything it wrote. That shape
 * cannot express "this input makes the job throw" or "a character that LOSES
 * its agent block is still in the soft-delete scope", so those live here with a
 * lighter prisma stub and a fresh run per case.
 */
import {
  beforeAll,
  beforeEach,
...
jest.mock("../db", () => ({
  prisma: {
    ...
    // The job also fills Character's SDE-owned columns and writes the
    // NpcCharacterSkill table; neither is what this file asserts on, so both
    // just need to answer.
    character: { ...delegate(), createMany: jest.fn() },
    npcCharacterSkill: { ...delegate(), createMany: jest.fn() },
  },
}));
jest.mock("../helpers", () => ({
  loadSdeFile,
