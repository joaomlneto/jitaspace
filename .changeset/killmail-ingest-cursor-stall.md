...
- References the database cannot satisfy are now resolved before the insert. A killmail missing a _required_ one (solar system, victim ship, item type) is dropped and logged; missing _optional_ ones (moon, attacker ship, attacker weapon) are nulled instead of discarding the kill.
