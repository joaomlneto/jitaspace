...
Station and constellation pages now show their name and details in the initial page load rather than filling in afterwards, and a station or constellation that does not exist now shows the not-found page — and tells search engines not to index it — instead of an empty page that looked real. `/about`, `/dogma` and `/search` finally have their own titles and descriptions instead of inheriting the generic site one.
