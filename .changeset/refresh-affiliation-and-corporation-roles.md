...
`useAccessToken` enforces `roles` exactly as it enforces `scopes`: a character is only eligible if its `corporationRoles` contains every required role. Roles that have never been read are an empty list, so such a character is excluded rather than tried on spec. Any page passing `roles` must therefore also require `esi-characters.read_corporation_roles.v1` in its `ScopeGuard`, or a user who never granted that scope gets no token and no explanation.
