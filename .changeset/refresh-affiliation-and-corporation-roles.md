...
`useAccessToken` excludes a character whose *known* roles fall short and prefers a known holder, but still falls back to a character whose roles have never been read — the user may never have granted the roles scope, and ESI enforces the roles server-side anyway.
