...
`useAccessToken` now enforces `roles`, which lists the roles that are _accepted_ rather than ones that must all be held: a character is eligible if its `corporationRoles` contains at least one of them. That mirrors ESI's own `x-required-roles`, an any-of list — five of its 34 role-gated operations accept either of two roles (corporation wallets take `Accountant` or `Junior_Accountant`). An empty or omitted list means no role is needed.

Roles that have never been read are an empty list, so such a character matches nothing and is excluded rather than tried on spec. Reading roles needs `esi-characters.read_corporation_roles.v1`; that scope is not forced on anyone, so a character without it holds no roles as far as this hook is concerned and will not authorise a role-gated request.
